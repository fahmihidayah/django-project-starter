...
class GeneratorClient:
    ...
    def write_target_model_v2(self, target_model: TargetModel):
        for file_item in target_model.result['result_files']:
            file_dir = file_item['dir']
            if file_dir:
                dirs = file_dir.split('/')
                path = target_model.directory
                for dir in dirs:
                    path = path / pathlib.Path(dir)
                    if not path.exists():
                        path.mkdir()
                write_string(path, '{}{}'.format(file_item['name'].lower(), file_item['extension']),
                             file_item['value'])
            else:
                write_string(target_model.directory, '{}{}'.format(file_item['name'], file_item['extension']), file_item['value'])
    ...
    def write_model(self):
        for target_model in self.target_models:
            ...
            self.write_target_model_v2(target_model)
    ...
    def generate_model_v3(self):
        response = self.request_generate_model()
        self.target_models = jsonpickle.decode(response.text)
        # print(self.target_models[0].result)
        # print(self.target_models)
        self.write_model()

    def request_generate_model(self):
        self.target_models = self.read_model()
        response = requests.post(self.url,
                                 data = {
                                     'username': self.username,
                                     'password': self.password,
                                     'models': jsonpickle.dumps(self.target_models)
                                 })
        return response
...
def test():
    ...
    results = generator_client.generate_model_v3()
